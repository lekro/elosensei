...
class Elo:
    ...
    async def get_elo(self, player):
        try:
            return self.user_status.loc[player, 'elo']
        except:
            self.user_status.loc[player] = (self.config['default_elo'], 0, 0)
            return self.config['default_elo']

    async def update_players(self, match_df):
        ...
        # Actual team scores
        team_elo['actual'] = team_elo['status'].head(1).applymap(self.get_status_value).tolist()
        k_factor = self.config['k_factor']
        team_elo['elo_delta'] = k_factor * (team_elo['actual'] - team_elo['expected'])
        
        for index, row in match_df.iterrows():
            self.user_status.loc[player, 'elo'] += team_elo.loc[row.team, 'elo_delta']
            actual_score = team_elo.loc[row.team, 'actual']
            if actual_score == 1:
                self.user_status.loc[player, 'wins'] += 1
            elif actual_score == 0:
                self.user_status.loc[player, 'losses'] += 1
